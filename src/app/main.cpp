...
////    QWindow* mainWin=app.allWindows().first();
////    mainWin->setFlags(Qt::FramelessWindowHint);
//    //engine.load(QUrl(QStringLiteral("qrc:/main.qml")));
////    QObject::connect(&engine,&QQmlApplicationEngine::objectCreated,[&](QObject* obj,const QUrl &url){
////        qDebug()<<obj->objectName()<<engine.rootObjects().size()<<url.toString();
////    });
//    QWindow* loginWin =qobject_cast<QWindow*>(engine.rootObjects().at(0));

//    //QWindow* loginWin=qobject_cast<QWindow*>(engine.findChild<QObject*>("Login Window"));
//    //loginWin->setFlags(Qt::FramelessWindowHint);
//    qDebug()<<engine.rootObjects().size()<<loginWin->objectName();
////    MyClass myClass;
//    QObject::connect(loginWin, SIGNAL(qmlSignal(QString)),
//                     &myClass, SLOT(cppSlot(QString)));
//    QObject::connect(&myClass,SIGNAL(setState(QVariant)),loginWin,SLOT(setState(QVariant)));

//    QObject::connect(&myClass, &MyClass::loginOk,
//                      [&](const QString &msg){

//        qDebug()<<"lambda slot test:"<<msg;
//        engine.load(QUrl(QStringLiteral("qrc:/main.qml")));
//    });
//    DICT::login();
 //   return app.exec();
...
//    //扇贝词典版本信息
//    DICT::versioninformation=QObject::tr("扇贝词典1.3.4");
//    //所有使用本软件全部或者部分代码的程序设计人员请保留lience信息，并在所发布的软件关于窗口中进行显示。
//    DICT::licenceinformation=QObject::tr("<h2>%1</h2>"
